...
start_diagnostic_backend() {
  ...
  echo "[diagnostic] start.sh failed or sd-server stopped (status=${status})" >&2
  echo "[diagnostic] keeping this Salad instance alive on 127.0.0.1:${DIAGNOSTIC_PORT}" >&2
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/v1/models":
            # bootstrap.sh uses this endpoint as its readiness check.
        ...
        self._write_json(503, {
            "error": "sd-server is not running",
            "diagnostic": True,
            "start_status": status,
        })
    ...
    def log_message(self, fmt, *args):
        print("[diagnostic] " + (fmt % args), flush=True)

print(f"[diagnostic] listening on http://{host}:{port}", flush=True)
...
on_error() {
  local status=$?
  start_diagnostic_backend "$status"
}
...
echo "[server] sd-server exited with status=${SD_STATUS}" >&2
start_diagnostic_backend "$SD_STATUS"
